...
public class BookOwnerFacilityWithOverlappingTimingsWithAdminLogin extends TestBase{
@Test(priority=1,dataProvider="OwnerFacilityOverlapWithAdminLogin",dataProviderClass= DataProvider5.class)
...
Reporter.log("ScriptName:Admin Login - Book for 9:30 AM to 11:30 AM - Same timing Overlap", true);	
Reporter.log("----------------------------------------------------------------------------", true);
Reporter.log(" ", true);
	...
	helper.TakeScreenShot("Search For OwnerFacility For Booking With OverLapTimings With Admin Login ");}
	try{
	...
	helper.MaxSleep();
	helper.TakeScreenShotOfWindowPopUp("Popup Came After saving details of Booking of OwnerFacility with Alreadybooked Timing With Admin Login");
	helper.DeepSleep();
	try{
	helper.ProcessAlert();
	helper.Sleep();
	...
	Reporter.log("---------------------------", true);	
	Reporter.log("File Name : "+GlobalVariablesCalling.ScreenShotsFileName+"Search For OwnerFacility For Booking With OverLapTimings With Admin Login " , true);
	Reporter.log("File Name : "+GlobalVariablesCalling.ScreenShotsFileName+"Popup Came After saving details of Booking of OwnerFacility with Alreadybooked Timing With Admin Login",true);
	Reporter.log(" ", true);

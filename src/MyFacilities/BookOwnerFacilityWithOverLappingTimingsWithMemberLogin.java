...
public class BookOwnerFacilityWithOverLappingTimingsWithMemberLogin extends TestBase{
@Test(priority=1,dataProvider="OwnerFacilityOverlapWithMemberLogin",dataProviderClass= DataProvider5.class)
...
Reporter.log("ScriptName:Member Login (Owner) - Book for 9:31 AM to 11:31 AM - Diff Timing but overlap", true);	
Reporter.log("----------------------------------------------------------------------------", true);
Reporter.log(" ", true);
...
helper.TakeScreenShot("Search For OwnerFacility For Booking With OverLapTimings With Member Login ");}
try{
...
helper.MaxSleep();
helper.TakeScreenShotOfWindowPopUp("Popup Came After saving details of Booking of OwnerFacility with Alreadybooked Timing With Member Login");
helper.DeepSleep();
try{
helper.ProcessAlert();
helper.Sleep();
...
Reporter.log("---------------------------", true);	
Reporter.log("File Name : "+GlobalVariablesCalling.ScreenShotsFileName+"Search For OwnerFacility For Booking With OverLapTimings With Member Login " , true);
Reporter.log("File Name : "+GlobalVariablesCalling.ScreenShotsFileName+"Popup Came After saving details of Booking of OwnerFacility with Alreadybooked Timing With Member Login",true);
Reporter.log(" ", true);
}

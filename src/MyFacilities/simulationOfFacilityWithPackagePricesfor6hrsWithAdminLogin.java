...
public class simulationOfFacilityWithPackagePricesfor6hrsWithAdminLogin extends TestBase{
	@Test(priority=1,dataProvider="SimulateFacilityWithpackagePricesfor6hoursWithAdminLogin",dataProviderClass= DataProvider7.class)
	...
	Reporter.log("ScriptName:Admin Login  - Book for 6 hours  (9 AM to 3 PM) - Should not allow for booking", true);	
	Reporter.log("-----------------------------------------------------------------------------------------", true);
	Reporter.log(" ", true);
	...
	helper.TakeScreenShot("Simulation Of Facility With package Prices For 6hrs With Admin Login");
	}}			
	}catch(NoSuchElementException Exception){
	Reporter.log("Booking Button Not Available So booking Is Not Allowed For This Facility", true);
	String PriceDetailsButton=FacilityColoumns.get(12).findElement(By.tagName("a")).getText();
	Reporter.log("Preice Details Button Is Exists With Name Of:" +PriceDetailsButton, true);
	helper.TakeScreenShot("Booking Not Allowed For Facility Withpackage Prices For 6hrs With Admin Login");
	Reporter.log("Booking Not Allowed For Facility With package Prices For 6hrs With Admin Login", true);
	}}catch(NoSuchElementException Exception){
	...
	Reporter.log("---------------------------", true);	
	Reporter.log("File Name : "+GlobalVariablesCalling.ScreenShotsFileName+"Simulation Of Facility With package Prices For 6hrs With Admin Login",true);
	Reporter.log(" ", true);
		}

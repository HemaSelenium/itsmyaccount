...
public class BookFreeFacilityfor30mintsOverlaping extends TestBase{
	
	@Test(priority=1,dataProvider="BookFreeFacilityWith30MintsSlot",dataProviderClass= DataProvider4.class)
	...
	Reporter.log("ScriptName:Member Login --> Book 30 min  (Eg: 9 AM to 9:30 AM) - Overlapping", true);	
	Reporter.log("----------------------------------------------------------------------------", true);
	Reporter.log(" ", true);
	...
	try{
		...
		helper.TakeScreenShot("Search For Free Facility To Check Overlapping");}
		try{
		...
		helper.ProcessAlert();
		helper.Sleep();
		helper.TakeScreenShotOfWindowPopUp("Booking Is allowed of Free Facility with Overlapping Timings");
		Reporter.log("Booking Is allowed of Free Facility with Overlapping Timings", true);
		}
		catch(NoAlertPresentException e){
		Reporter.log("Booking Is Not Allowed", true);
		Reporter.log("Error Message CameAs: "+helper.GetValueByID("InvailLabel"), true);
		helper.TakeScreenShotOfWindowPopUp("Booking Is Not allowed of Free Facility with Overlapping Timings");
	...
	Reporter.log("---------------------------", true);	
	Reporter.log("File Name : "+GlobalVariablesCalling.ScreenShotsFileName+"Search For Free Facility To Check Overlapping" , true);
	Reporter.log("File Name : "+GlobalVariablesCalling.ScreenShotsFileName+"Booking Is Not allowed of Free Facility with Overlapping Timings",true);
	Reporter.log(" ", true);

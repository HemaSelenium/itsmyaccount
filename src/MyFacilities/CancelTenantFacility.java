...
public class CancelTenantFacility extends TestBase{
@Test(dataProvider="CancelTenantFacility",dataProviderClass=DataProvider6.class)
public void CancelFacility(String AdddedFacilityName,String FacilityName,String DescriptionOfFacility) throws InterruptedException, IOException, HeadlessException, AWTException{
	Reporter.log("ScriptName:Member Login (Tenant) - Community (T) Booked for 2 hours (10:01 to 12:00 PM) - Cancel", true);	
	Reporter.log("----------------------------------------------------------------------------------------", true);
	Reporter.log(" ", true);
	...
	helper.DeepSleep();
	helper.TakeScreenShotOfWindowPopUp("AfterClickedOnCancelButtonOfTenantFacility");
	try{
	helper.ProcessAlert();
	helper.MaxSleep();
	helper.TakeScreenShot("FacilityCalendarAfterCancelTenantFacility");
	...
	Reporter.log("---------------------------", true);	
	Reporter.log("File Name : "+GlobalVariablesCalling.ScreenShotsFileName+"AfterClickedOnCancelButtonOfTenantFacility", true);

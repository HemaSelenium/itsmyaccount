...
public class BookOwnerFacilityWithOwnerLoginAfterCancel extends TestBase{
	@Test(priority=1,dataProvider="BookOwnerFacilityAfterCancelledFacility",dataProviderClass= DataProvider6.class)
	public void BookingFacility(String EnterFacilityNameToSearch,String EnterFacilityName,String EnterFromTime,String EnterToTime,String EnterDescription) throws InterruptedException, IOException, HeadlessException, AWTException{
	Reporter.log("ScriptName:Member Login (Owner) - Community Hall (O) - Book for 1:30 hours (9:30 AM to 11:00 AM) - Charge should be 2000", true);	
	Reporter.log("----------------------------------------------------------------------------", true);
	Reporter.log(" ", true);
	...
	if(FacilityNames.equals(EnterFacilityName)){
	Reporter.log("Added Free Facility Found Sucessfully", true);
	helper.TakeScreenShot("Search For OwnerFacility Aftercancel With MemberLogin");}
	try{
	...
	helper.MaxSleep();
	helper.TakeScreenShotOfWindowPopUp("PopupCameAfter saving details of Booking of OwnerFacility after cancelation with Specified Timings With MemberLogin");
	helper.MaxSleep();
	try{
	helper.ProcessAlert();
	helper.Sleep();
	...
	Reporter.log("---------------------------", true);	
	Reporter.log("File Name : "+GlobalVariablesCalling.ScreenShotsFileName+"Search For OwnerFacility Aftercancel With MemberLogin" , true);
	Reporter.log("File Name : "+GlobalVariablesCalling.ScreenShotsFileName+"PopupCameAfter saving details of Booking of OwnerFacility after cancelation with Specified Timings With MemberLogin",true);
	Reporter.log(" ", true);

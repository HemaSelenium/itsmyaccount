...
public class ExpectedVisitorWithBlockAndApartmentWithoutSMStick extends TestBase1 {
String VisitorName;
String choose="yes";
	...
	public void addWithoutBlockAndapartment(String Visitorname, String contactperson, String contactnum,String date,String Fromtime, String Totime,String Purpose ) throws AWTException, InterruptedException{
		Reporter.log("Test Script:  VisitorregisterTestingIMA_TC_007",true);
		Reporter.log("--------------------------------------",true);
		Reporter.log("Script Name: ExpectedVisitorWithBlockAndApartmentWithoutSMStick",true);
		...
		SeleniumHelper1.driver.findElement(By.xpath(Variables.Blockfield)).clear();
		helper1.SAP();
		helper1.SetValueByxpath(Variables.Blockfield, Variables.Block);
		helper1.SAP();
		helper1.SetValueByxpath(Variables.Apartmentfield, Variables.Apartment);
		helper1.SAP();
		helper1.SAP();
	...
	public Object[][] readexcel2() throws IOException, BiffException {
		...
		String inputdata[][] = new String[rows - 2][columns-19];
		for (int i = 2; i < rows; i++) {
			for (int j = 19; j < columns; j++) {
				Cell cl = s.getCell(j, i);
				inputdata[i - 2][j-19] = cl.getContents();
			}
		}
		return inputdata;
	}
	...
	public Object[][] readexcel21() throws IOException, BiffException {
		...
		String inputdata[][] = new String[rows - 4][columns-4];
		for (int i = 4; i < rows; i++) {
			for (int j = 4; j < columns; j++) {
				Cell cl = s.getCell(j, i);
				inputdata[i - 4][j-4] = cl.getContents();
			}
		}
		return inputdata;
	}
